...
function answered(instance,attached,request){console.log("instance action:"+ instance.action);replied=true;if(attached!='undefine'&&attached!='nul'&&attached!==undefined&&attached){eval(attached);}
if(request!='undefine'&&request!='nul'&&request!==undefined&&request){eval(request.responseText);}
if(instance.href){window.location=instance.href;}else if(instance.action&&instance.push==undefined){console.log("relying on addEvent to call properly");}}
...
console.log("eval "+forms[counter].id);f=document.getElementById(forms[counter].id);eval('formSubmitFunctions["'+forms[counter].id+'"] = function(){SendData("'+forms[counter].id+'", '+forms[counter].onsubmit+');return false;};');addEvent(forms[counter],'submit',function(){console.log("sending data - counter: "+counter+" - forms: "+document.getElementsByTagName('form')+" - id: "+document.getElementsByTagName('form')[counter].id);SendData(forms[counter].id,forms[counter].onsubmit);console.log("sent data");return true;});console.log(forms[counter].elements);for(var i=0;i<forms[counter].elements.length;i++){if(forms[counter].elements[i].type=='submit'){console.log('submit button');addEvent(forms[counter].elements[i],'click',function(){this.form.submited=this.value;});}}}}}
