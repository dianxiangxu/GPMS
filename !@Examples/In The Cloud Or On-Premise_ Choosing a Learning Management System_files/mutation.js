...
var TreeMirrorClient = (function () {
    function TreeMirrorClient(target, mirror, testingQueries) {
        ...
        var rootId = this.serializeNode(target).i;
        var children = [];
	...
	TreeMirrorClient.prototype.reconnect = function(){
    ...
    TreeMirrorClient.prototype.serializeNode = function (node, recursive) {
        ...
        switch (data.n) {
            ...
            case Node.COMMENT_NODE:
            case Node.TEXT_NODE:
				// don't allow textboxes with pre-entered values sent
				if (node.parentElement && node.parentElement.getAttribute("type") && typeof(node.parentElement.getAttribute("type")) == 'string' && node.parentElement.getAttribute("type").toLowerCase() == 'textarea')
				{
					// sensitive so make text all asterisks
					var asterisks = '';
				...
				{
					// this is just a text element on the page
					// maybe text elements are sensitive so allow them to 
					// add the LoSensitive Class and then we won't transmit it
					// see if LOSensitive is here or on parent
					var node_class = "";
					var node_parent_class = "";
					
					var node_class_type = typeof(node.className);
					if (node_class_type.toLowerCase() == 'string')
					{						
						node_class = node.className.toLowerCase();
					}
					
					if (node.parentElement)
					{
						var node_parent_class_type = typeof(node.parentElement.className);
						if (node_parent_class_type.toLowerCase() == 'string')
						{						
							node_parent_class = node.parentElement.className.toLowerCase();
						}
					}
					
				
					
					if ( node_class.indexOf("losensitive") > -1 || node_parent_class.indexOf("losensitive") > -1 )
					{
						// sensitive so make text all asterisks
						var asterisks = '';
						for (var k=0; k<node.textContent.length; k++)
            ...
            case Node.ELEMENT_NODE:
				...
				var dont_allow_val = false;
				var el_type = '';
				if (elm && elm.nodeName)
				{
					el_type = elm.nodeName.toLowerCase();
					
				}
				if (el_type == 'script')
				{
					// don't need script attributes
					
					return data;
				}
				else if (el_type == 'input' || el_type == 'textarea')
				{
					dont_allow_val = true;
					if (el_type == 'input' && elm.type )
					{
						// has a type attribute, see if this is a button
						var el_attr_type_sm = elm.type.toLowerCase();
						if (el_attr_type_sm == 'submit' || el_attr_type_sm == 'button')
						{
							dont_allow_val = false; // submit and buttons allowed
						}
					}
                ...
                for (var i = 0; i < elm.attributes.length; i++) {
					...
					if (dont_allow_val && attr.name && attr.name.toLowerCase() =='value' )
					{
						// leave values out for privacy concerns
						data.a[attr.name] = '';
					}
					else
					{
                ...
                if (recursive && elm.childNodes.length) {
                    ...
                    for (var child = elm.firstChild; child; child = child.nextSibling)
                        data.c.push(this.serializeNode(child, true));
                }
                break;
        }

			// to save space in transmission we shorten the property names
			/*
				data.g = tagName;
				data.a = attributes;
				data.t = textContent;
