...
var s_account="";
if (typeof omniture_account != 'undefined') {
	s_account = omniture_account;
}
...
s.linkInternalFilters="javascript:,www.pcworld.com,macworld.com,www.macworld.com,hints.macworld.com"
...
function s_doPlugins(s) {
	...
	s.prop45=s.getPercentPageViewed();s.eVar45="D=c45";
	
	/*Days since last login*/
	s.eVar34=s.getDaysSinceEvent('event20');s.prop8="C=d34";
	
	/* New Vs. Repeat */	
	s.prop64=s.getNewRepeat(5000);s.eVar63="D=c64";

	/* Visit Number  */
	s.prop60=s.getVisitNum(5000);s.eVar62="D=c60";

	/*Days Since Last Visit*/
	s.prop59=s.getDaysSinceLastVisit();s.eVar67="D=c59";
...
s.Media.playerName="NarfPlayer - SiteCatalyst 15";
s.Media.segmentByMilestones = true;
s.Media.trackUsingContextData = true;
s.Media.trackVars = "eVar2,eVar52,eVar50,prop52";
s.Media.trackEvents = "event1,event2,event10,event12,event13,event14,event15,event39,event23,event24,event25,event26"; 
...
s.getDaysSinceEvent=new Function("e","cn",""
+"if(!e){return''}var s=this;cn=cn&&cn!=''?cn:'s_gdse_'+e;var cv;var "
+"cn_visit=cn+'_visit';var s_invisit=s.c_r(cn_visit);var d=new Date()"
+";var ct=d.getTime();var ex=new Date();ex.setTime(ct+3*365*day);var "
+"day=24*60*60*1000;if((s.events+',').indexOf(e+',')>-1){cv=s.c_r(cn)"
+";s.c_w(cn,ct,ex);if(!s_invisit){s.c_w(cn_visit,'true',0);if(cv){var"
+" days=Math.floor((ct-cv)/day);if(days==0){var hours=Math.floor(((ct"
+"-cv)/day)*24);if(hours==0){days='same visit'}else{days=hours+' days"
+"'}}return days}else{return'first visit'}}}");
/*
...
if(typeof omniture_namespace != 'undefined') {
	s.visitorNamespace=omniture_namespace;
} else { s.visitorNamespace="idgenterprise"; }
	
if(typeof omniture_trackingServer != 'undefined') {
	s.trackingServer=omniture_trackingServer;
} else { s.trackingServer="idgenterprise.d1.sc.omtrdc.net"; }
s.dc="122";
...
+"gServerBase,p='.sc',ns=s.visitorNamespace,un=s.cls(u?u:(ns?ns:s.fun)),r=new Object,l,imn='s_i_'+s._in+'_'+un,im,b,e;if(!rs){if(t1){if(t2&&s.ssl)t1=t2}else{if(!tb)tb='omtrdc.net';if(dc)dc=(''+dc).toLow"
+"erCase();else dc='d1';if(tb=='omtrdc.net'){if(dc=='d1')dc='112';else if(dc=='d2')dc='122';p=''}t1=un+'.'+dc+'.'+p+tb}rs='http'+(s.ssl?'s':'')+'://'+t1+'/b/ss/'+s.un+'/'+(s.mobile?'5.1':'1')+'/'+s.vers"
