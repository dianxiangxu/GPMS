...
(function($) {
  ...
  Drupal.cp_collapsiblock = Drupal.cp_collapsiblock || {};

  Drupal.behaviors.cp_collapsiblock = {

    attach: function (context,settings) {
      
      var cookieData = Drupal.cp_collapsiblock.getCookieData();
      ...
      $(block + ':not(.collapsiblock-processed)', context).addClass('collapsiblock-processed').each(function () {
        ...
        if (titleElt.size()) {
          ...
          //check if mobile only or desktop only collapsible behaviour
           var ww = jQuery(window).width();
          if(settings.cp_collapsiblock.cp_collapse_mobile_status[id]==2){
              if(ww>767){
                    return;
             }
          }    
          if(settings.cp_collapsiblock.cp_collapse_mobile_status[id]==3){
              if(ww<=767){
                    return;
             }
          } 
          ...
          .click(function (e) {
            var st = Drupal.cp_collapsiblock.getCookieData();
  ...
  Drupal.cp_collapsiblock.getCookieData = function () {
    if ($.cookie) {
      var cookieString = $.cookie('cp_collapsiblock');
