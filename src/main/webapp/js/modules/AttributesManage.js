...
$(function() {
	...
	attributesManage = {
		config : {
			...
			data : '{}',
			dataType : 'json',
			baseURL : AspxCommerce.utils.GetAspxServicePath() + "jsonServices/",
			method : "",
			url : "",
			ajaxCallMode : 0
		},
		ajaxCall : function(config) {
			$.ajax({
				...
				beforeSend : function(request) {
					...
					request.setRequestHeader("UName", AspxCommerce.utils
							.GetUserName());
					request.setRequestHeader("PID", AspxCommerce.utils
							.GetPortalID());
		...
		LoadAttributeStaticImage : function() {
			$('.cssClassSuccessImg').prop('src',
					'' + aspxTemplateFolderPath + '/images/right.jpg');
		},
		...
		GetValidationTypeErrorMessage : function(attValType) {
			...
			switch (attValType) {
			case "1":
				retString = getLocale(AspxAttributesManagement,
						'Alphabets Only');
				break;
			case "2":
		...
		ClearForm : function() {
			...
			$('#' + lblAttrFormHeading).html(
					getLocale(AspxAttributesManagement, "New Item Attribute"));
		...
		ValidationTypeEnableDisable : function(fillOptionValues, isChanged) {
			...
			case "1":
				...
				$('#' + lblDefaultValue).html(
						getLocale(AspxAttributesManagement, "Default Value:"));
				$('#' + lblLength).html(
						getLocale(AspxAttributesManagement, "Length:"));
				if (isChanged) {
					$('#txtLength').val('');
				}
				$("#txtLength").removeAttr('disabled');
			...
			case "5":
				...
				$("#ddlTypeValidation").prop('disabled', 'disabled');
				$('#' + lblLength).html(
						getLocale(AspxAttributesManagement, "Size:"));
				$("#txtLength").removeAttr('disabled');
				$('#txtLength').next('span').next('span').show();
				if (isChanged) {
					$('#txtLength').val(3);
			...
			case "8":
				$('#ddlTypeValidation').val('8');
				$('#' + lblDefaultValue).html(
						getLocale(AspxAttributesManagement,
				...
				$('#' + lblLength).html(
						getLocale(AspxAttributesManagement, "Size:(KB)"));
				if (isChanged) {
					$('#txtLength').val('');
				}
				$("#ddlTypeValidation").prop('disabled', 'disabled');
			...
			case "12":
				...
				$(".tddefault")
						.html(
								'<input type=\"checkbox\" name=\"defaultChk\" class=\"class-isdefault\">');
				$(".AddOption").val(
						getLocale(AspxAttributesManagement, "Add More"));
				$(".AddOption").show();
				attributesManage.BindAttributeOptionsValues(fillOptionValues);
				$('input[name=chkIsEnableEditor]').prop('disabled', 'disabled');
				break;
		...
		BindAttributeGrid : function(attributeNm, required, SearchComparable,
			...
			this.config.method = "GetUserAccountList";
			var offset_ = 1;
			var current_ = 1;
								...
								buttons : [
										{
											...
											_event : 'click',
											trigger : '1',
											callMethod : 'attributesManage.EditAttributes',
										...
										},
										{
											...
											_event : 'click',
											trigger : '2',
											callMethod : 'attributesManage.DeleteAttributes',
											arguments : '5'
										},
										{
											display : getLocale(
											...
											_event : 'click',
											trigger : '4',
											callMethod : 'attributesManage.ActiveAttributes',
											arguments : '5'
										},
										{
											display : getLocale(
											...
											_event : 'click',
											trigger : '5',
											callMethod : 'attributesManage.DeactiveAttributes',
											arguments : '5'
										} ],
								rp : perpage,
								nomsg : getLocale(AspxAttributesManagement,
										'No Records Found!'),
								param : {
		...
		FillForm : function(response) {
			$.each(response.d, function(index, item) {
				...
				attributesManage.ValidationTypeEnableDisable(
						item.FillOptionValues, false);
		...
		EditAttributes : function(tblID, argus) {
			...
			case "gdvAttributes":
				$('#languageSelect').find('li').each(
						function() {
							if ($(this).attr("value") == AspxCommerce.utils
									.GetCultureName()) {
				...
				attributesManage.config.url = attributesManage.config.baseURL
						+ "GetAttributeDetailsByAttributeID";
				attributesManage.config.data = JSON2.stringify({
					attributeId : argus[0],
					aspxCommonObj : aspxCommonObj()
				});
		...
		RebindAttributesOnLanguageChange : function() {
			...
			if ($("#languageSelect").length > 0) {
				aspxCommonInfo.CultureName = $(".languageSelected").attr(
						"value");
			}
			// attributesManage.onInit();
			attributesManage.config.url = attributesManage.config.baseURL
			...
			$("#default_value_text").prop(
					...
					"sfInputbox "
							+ attributesManage
									.CreateValidationClass(attValType) + "");
			$('#iferror').html(
					attributesManage.GetValidationTypeErrorMessage(attValType));
		},
		...
		SaveAttribute : function(_attributeId, _flag) {
			...
			if (checkForm($("#form1"))) {
				...
				} else if (!attributesManage.IsUnique(attributeName,
						_attributeId)) {
				...
				if (!validateErrorMessage) {
					...
					attributesManage.AddAttributeInfo(_attributeId,
							_attributeName, _inputTypeID, _DefaultValue,
							_ValidationTypeID, _Length, _AliasName,
							_AliasToolTip, _AliasHelp, _DisplayOrder,
							_IsUnique, _IsRequired, _IsEnableEditor,
							_ShowInAdvanceSearch, _ShowInComparison,
							_IsUseInFilter, _IsIncludeInPriceRule,
							_IsShowInItemListing, _IsShowInItemDetail,
							_StoreID, _PortalID, _IsActive, _IsModified,
							_UserName, _CultureName, _ItemTypes, _Flag,
							_IsUsedInConfigItem, _saveOptions,
							_attributeValueId);
		...
		SearchAttributeName : function() {
			...
			attributesManage.BindAttributeGrid(attributeNm, required,
		...
		ajaxFailure : function(msg) {
			...
			case 1:
				csscody.error('<h2>'
						+ getLocale(AspxAttributesManagement, "Error Message")
						+ '</h2><p>'
						+ getLocale(AspxAttributesManagement,
		...
		init : function(config) {
					...
					.click(
							function() {
								...
								if ($("#btnSaveAttribute").prop("name") != "") {
									attributesManage
											.RebindAttributesOnLanguageChange();
								}
			...
			$('#btnDeleteSelected')
					.click(
							function() {
								var attribute_ids = '';
								attribute_ids = SageData.Get("gdvAttributes").Arr
										.join(',');
			...
			$('#txtAttributeName')
					.blur(
							function() {
								...
								} else if (!attributesManage.IsUnique(
